...
命名空间键不能包含字符“.”，因为该字符用于表示嵌套——其他字符均可使用。
...
如果你之前使用可读性较强的句子作为键，理论上可以在将消息传给 `next-intl` 之前，将字符“.”映射为其他字符（例如“_”），但通常建议使用 ID 作为键。如果你使用可读性较强的键主要是为了在编辑器中查看消息中的可读标签，那么仍然可以使用[编辑器集成](https://learn.next-intl.dev/chapters/03-translations/05-tooling)来实现这一点。
...
```js
t('message', {count: 3580}); // "你有 3,580 位关注者。"
```
...
例如，英文有两种形式：单数（如“1 位关注者”）和其他（如“0 位关注者”、“2 位关注者”）。中文只有一种形式，阿拉伯语则有六种。

另一方面，从可用性考虑，常会考虑增加特殊零的情况（如“还没有关注者”替代“0 位关注者”）。
...
- `few`：适用于少数项目语法的语言（如阿拉伯语、克罗地亚语）。
- `many`：适用于大量项目语法的语言（如阿拉伯语、克罗地亚语）。
- `other`：用于不匹配其它类别的情况。
